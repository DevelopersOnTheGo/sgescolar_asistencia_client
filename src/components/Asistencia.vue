<template>
  <v-container grid-list-md >
    ...
    <v-layout row wrap>
      ...
      <v-flex xs4>
        <v-date-picker @click="send(picker)" locale :color="`${theme.defaulttheme.color.first}`" v-model="picker" :header-color="`${theme.defaulttheme.color.first}`"></v-date-picker>
      </v-flex>
      <v-flex >
        <div>
          <v-btn flat v-on:click="send(picker)">Guardar Asistencia</v-btn>
        </div>
      </v-flex>
    </v-layout>
  </v-container>
...
<script>
  ...
  export default {
    ...
    methods: {
      sendData() {
        this.$store.dispatch('SAVE_ASISTENCIA_ESTUDENT');
      },
      send(id) {
        console.log(id);
      },
